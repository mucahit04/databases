...
async function seedDatabase() {
  const create_papers_table =
    "CREATE TABLE IF NOT EXISTS Research_Papers (paper_id INT PRIMARY KEY, paper_title TEXT, conference TEXT, publish_date DATE);";

  const create_relation_table =
    "CREATE TABLE IF NOT EXISTS Relation_Table (author_id INT, paper_id INT, CONSTRAINT FK_Author FOREIGN KEY(author_id) REFERENCES authors(author_no), CONSTRAINT FK_Paper FOREIGN KEY(paper_id) REFERENCES Research_Papers(paper_id), CONSTRAINT PK_Author_Paper PRIMARY KEY(author_id, paper_id));";

  function random_collab_number() {
    return (
      101 +
      Math.floor(
        Math.random() * collaborators.length
      )
    );
  }
  ...
  try {
    ...
    relation_data.forEach(async (data) => {
      await execQuery(
        "INSERT INTO Relation_Table SET ?",
        data
      );
    });
